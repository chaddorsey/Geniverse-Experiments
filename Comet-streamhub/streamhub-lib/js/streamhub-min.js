...
},connectFrames:function(A){if(StreamHub.Browser.isIEFamily()){this.eResponseIFrame=this.addIFrame({id:"responseIFrame"+new Date().valueOf(),src:""});
...
if(StreamHub.Browser.isFirefoxFamily()&&!StreamHub.Browser.isFF3()){window.l=this.buildConnectionLost(this)
...
}var A=this.pRequestQueue[0].url;
this.oLogger.log("Requesting: "+A);
if(StreamHub.Browser.isIEFamily()){this.eRequestIFrame.src=A
}else{this.eContainerIFrame.contentWindow.request(A,this)
...
}},isResponseIFrameConnected:function(){return this.bIsResponseIFrameConnected
...
},buildCleanUp:function(A){return function(){try{A.bConnectCalled=false;
